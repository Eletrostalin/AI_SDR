...
async def process_message(message, bot):
    """
    Определяет тип сообщения: текст, файл или ссылка, и извлекает данные.

    :param message: Сообщение от пользователя.
    :param bot: Экземпляр бота (для скачивания файлов).
    :return: Словарь с типом и содержимым сообщения.
    """
    if message.text:
        if "http://" in message.text or "https://" in message.text:  # Если это ссылка
            try:
                content = extract_text_from_url(message.text)
                return {"type": "link", "content": content}
            except Exception as e:
                return {"type": "error", "message": f"Ошибка извлечения текста из ссылки: {e}"}
        else:
        ...
        try:
            content = extract_text_from_document(file_path, message.document.file_name)
            return {
                "type": "file",
                "mime_type": mime_type,
            ...
            return {"type": "error", "message": f"Ошибка обработки файла: {e}"}
    else:
