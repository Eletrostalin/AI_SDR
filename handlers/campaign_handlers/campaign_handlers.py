...
async def process_campaign_name(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.bot.send_message(
            ...
            text="Укажите по какому критерию из базы емейлов Вы хотите провести рассылку?\n\n"
                 "Введите ответ в формате: Критерий - Значение.\n"
                 "(Доступны только те поля, в которых заполнено хотя бы одно значение):\n"
                 "Название компании, Регион, Реестр МСП, Имя директора, Должность директора, Телефон, Email, Веб-сайт, Основная деятельность, Выручка, Количество сотрудников.\n\n"
                 "Вы можете выбрать одно или несколько полей. Например, если вы введёте:\n"
                 "\"Регион - Москва\n"
                 "Имя директора - Сергей\"\n"
                 "Рассылка будет отправлена на email Московских компаний, директора которых зовут Сергей."
        )
...
async def process_filters(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not filters:
            logger.warning("⚠️ Не удалось определить фильтры.")
        ...
        if not company_id or not campaign_id or not email_table_id:
            logger.error("❌ Кампания или email-таблица не найдена.")
        ...
        with SessionLocal() as db:
            ...
            logger.info(f"🔹 Количество строк после фильтрации: {len(filtered_df)}")

            if filtered_df.empty:
                logger.warning("⚠️ По заданным фильтрам не найдено ни одной записи.")
                await message.reply("⚠️ По заданным фильтрам не найдено ни одной записи.")
                return

            # 🔹 Обновляем фильтры кампании в БД
            update_status = update_campaign_filters(db, campaign_id, filters)
            logger.info(f"🔹 Обновление фильтров в БД: {'Успешно' if update_status else 'Ошибка'}")
